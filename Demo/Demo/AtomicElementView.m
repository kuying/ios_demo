...
- (void)drawRect:(CGRect)rect {
    ...
    NSDictionary *dict = [NSDictionary dictionaryWithObjects:[NSArray arrayWithObjects:[UIFont boldSystemFontOfSize:36], nil] forKeys:[NSArray arrayWithObjects:NSFontAttributeName, nil]];
    
	// draw the element name
	CGSize stringSize = [self.element.name sizeWithAttributes:dict];
    ...
    dict = [NSDictionary dictionaryWithObjects:[NSArray arrayWithObjects:[UIFont boldSystemFontOfSize:48], nil] forKeys:[NSArray arrayWithObjects:NSFontAttributeName, nil]];
	point = CGPointMake(10,0);
	[[NSString stringWithFormat:@"%@", self.element.atomicNumber] drawAtPoint:point withAttributes:dict];
	
	// draw the element symbol
    dict = [NSDictionary dictionaryWithObjects:[NSArray arrayWithObjects:[UIFont boldSystemFontOfSize:96], nil] forKeys:[NSArray arrayWithObjects:NSFontAttributeName, nil]];
	stringSize = [self.element.symbol sizeWithAttributes:dict];
...
- (UIImage *)reflectedImageRepresentationWithHeight:(NSUInteger)height {
    ...
    mainViewContentContext = CGBitmapContextCreate (NULL, self.bounds.size.width,height, 8,0, colorSpace, (CGBitmapInfo)kCGImageAlphaPremultipliedLast);
